...
`script-src` of either Content-Security-Policy that allowlists it — the `<meta>`
baseline injected into the built HTML and the `.htaccess` header superset — or if
the two policies disagree on any other directive (it reads the policy Apache
...
guard
(`npm run check:og`) that verifies `public/og-image.png` is the size
`index.html` advertises to link unfurlers, a deploy
