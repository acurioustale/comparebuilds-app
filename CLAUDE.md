...
node scripts/checkWireLayout.js              # confirm the build-string node order against Raidbots talents.json; no credentials, --env=ptr/beta looks at the next patch
...
`scripts/checkWireLayout.js` is the narrower companion check, and the only upstream check that runs on fork PRs. It compares just the build-string node ordering against Raidbots' published `talents.json` — an **independent derivation of the same client** (SimC casc/dbc tools), so it is outside evidence rather than the self-comparison the committed snapshot performs. No credentials (plain public GET, cached under `scripts/.cache/raidbots/`), and `--env=ptr`/`--env=beta` read the next patch's node set so a wire-layout break surfaces before it ships. `live` mismatch → exit non-zero; `ptr`/`beta` mismatch → warn only (divergence is expected while a patch is in test), and an unpublished test channel (404) is a no-op. Runs in `sources.yml` as the `wire-layout` job.
