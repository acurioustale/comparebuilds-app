...
The `deploy` job `needs: validate`, so a red gate never ships (deploy skipped on PRs). After a successful deploy, `deploy.sh` runs `api/cron/ensure_schema.php` over SSH to apply pending schema migrations. `./validate.sh` mirrors the gate locally — shfmt, php-cs-fixer, phpunit and actionlint are downloaded at their `.tool-versions` pin into a gitignored `.tools/` cache, exactly as CI does, so a `brew upgrade` can't drift a tool out from under its pin; an unavailable download falls back to the still-version-asserted PATH copy. The CLIs with no pinned download (shellcheck, xmllint, vnu) are skipped with a notice when absent; npm tools always run.
...
- **Toolchain is pinned in `.tool-versions`**: nodejs 26, php 8.4, plus shfmt/php-cs-fixer/phpunit/actionlint versions. CI reads it via `setup-node`'s `node-version-file`; match it when reproducing CI locally. `npm` (package-lock.json) — CI installs with `npm ci`. The **php pin tracks the production server, not the newest release** (8.4 is the latest it supports), so never bump it to chase a new PHP — `validate.sh` enforces an exact major.minor match. On macOS `php@8.4` is keg-only, so a linked `php` 8.5 shadows it and `validate.sh` aborts before running anything; put `/opt/homebrew/opt/php@8.4/bin` first on `PATH`. That runtime pin is the one brew can still break: since #448 the four pinned _tools_ are fetched, not taken from brew, so only the PHP engine itself has to be right locally.
